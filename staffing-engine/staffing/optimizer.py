...
Contrainte : Σ agents·productivité ≥ ETP requis, sur chaque créneau et chaque level.
...
from . import db
from .model import build_forecast_demand, month_local_dates, required_by_level
...
def optimize_allocation(month: str, db_path=db.DB_PATH, percentile: float = 1.0,
    ...
    """Résout l'IP par jour de semaine et renvoie l'allocation (dow, slot_utc, team, agents)."""
    solver = pulp.PULP_CBC_CMD(msg=0, timeLimit=time_limit, gapRel=gap_rel)
    demand = build_forecast_demand(month, db_path)
    required = required_by_level(demand, db_path)
    ...
    # ETP requis représentatif par (dow, slot, level) : percentile sur les dates du même dow.
    rep = (required.groupby(["dow", "slot_utc", "level"])["required_fte"]
    ...
    cost = dict(zip(teams["team_id"], teams["hourly_cost"]))
    ...
    for dow, rep_date in rep_dates.items():
        ...
        # index slot -> shifts couvrants, par équipe
        ...
        for team, shifts in cand.items():
            ...
            for sid, L, _cover in shifts:
                x[(team, sid)] = pulp.LpVariable(f"x_{team}_{sid}", lowBound=0, upBound=ub, cat="Integer")
        if not x:
            continue
        prob += pulp.lpSum(var * L * BUCKET_HOURS * cost[team]
                           for (team, sid), var in x.items()
                           for (_s, L, _c) in cand[team] if _s == sid)

        req_d = {(int(r.slot_utc), int(r.level)): r.required_fte
                 for r in rep[rep["dow"] == dow].itertuples()}
        levels = sorted(set(tlevel.values()))
        for slot in range(BUCKETS_PER_DAY):
            for level in levels:
                need = req_d.get((slot, level), 0.0)
                if need <= 0:
                    continue
                terms = [x[(team, sid)] * prod[team]
                         for team in cand if tlevel[team] == level
                         for (sid, _L) in cover_idx[team][slot]]
                if terms:
                    prob += pulp.lpSum(terms) >= need
            # plafond d'effectif par équipe et par créneau
            for team in cand:
                if pd.notna(cap[team]) and cover_idx[team][slot]:
                    prob += pulp.lpSum(x[(team, sid)] for (sid, _L) in cover_idx[team][slot]) <= int(cap[team])

        prob.solve(solver)
        for team in cand:
            for slot in range(BUCKETS_PER_DAY):
                agents = sum((x[(team, sid)].value() or 0) for (sid, _L) in cover_idx[team][slot])
                if agents > 0:
...
def expand_allocation(month: str, db_path=db.DB_PATH) -> pd.DataFrame:
    ...
    teams = db.read_table("team", db_path)
    required = required_by_level(build_forecast_demand(month, db_path), db_path)
    ...
    if alloc.empty:
        return pd.DataFrame(columns=["bucket_utc", "team_id", "level", "agents",
                                     "effective_capacity", "cost"])
    sup = buckets.merge(alloc, on=["dow", "slot_utc"]).merge(
        teams[["team_id", "level", "productivity", "hourly_cost"]], on="team_id")
    sup["effective_capacity"] = sup["agents"] * sup["productivity"]
    sup["cost"] = sup["agents"] * BUCKET_HOURS * sup["hourly_cost"]
    return sup[["bucket_utc", "team_id", "level", "agents", "effective_capacity", "cost"]]
...
def build_coverage(month: str, db_path=db.DB_PATH) -> dict:
    ...
    sup = (supply_team.groupby(["bucket_utc", "level"], as_index=False)[["effective_capacity", "agents", "cost"]].sum()
           if not supply_team.empty else
           pd.DataFrame(columns=["bucket_utc", "level", "effective_capacity", "agents", "cost"]))
    m = required.merge(sup, on=["bucket_utc", "level"], how="left")
    ...
    return {"matching": m.sort_values(["level", "bucket_utc"]).reset_index(drop=True),
