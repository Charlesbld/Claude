...
L'appli amorce la base SQLite si besoin, puis propose :
  ...
  • Demande — Réel vs Forecast (comparaison + édition du contact rate)
  • Couverture & coûts (Erlang C, optimiseur, heatmap, trous, répartition éditable)
  • Explorateur de données (pivots type BI sur toutes les tables)
  • Éditer les données
"""
...
def render_home():
    ...
    st.markdown(
        ...
        "**au quart d'heure**, en **UTC**, avec mutualisation de la capacité entre régions "
        "au sein d'un même *level* (Level 1 = prestataires externes, Level 2 = escalades internes).")
    ...
    digraph { rankdir=LR; bgcolor="transparent";
      ...
      TEAM[label="Équipes + disponibilités"]; OPT[label="Optimiseur IP\\n(coût minimal)"];
      ALLOC[label="Répartition d'agents\\n(éditable)"];
      COV[label="Couverture · coûts · trous", fillcolor="#FDEBD0", color="#E67E22"];
      PAX->DEM; CR->DEM; DEM->ERL; ERL->COV; TEAM->OPT; ERL->OPT; OPT->ALLOC; ALLOC->COV;
    }""")
    ...
    with c2:
        st.markdown(
            "**Dimensionnement (Erlang C)** — pour chaque bucket : nb d'agents pour tenir le SLA "
            ...
            "couvrant le requis (par jour de semaine). Résultat **éditable**.")

    st.info("👉 Commencez par **Couverture & coûts**, lancez l'optimiseur, puis explorez. "
            "Toutes les tables sont visibles dans **Explorateur** et modifiables dans **Éditer**.")
    with st.expander("Hypothèses de modélisation"):
        st.markdown(
            ...
            "(traduction automatique → pas de routage par langue).\n"
